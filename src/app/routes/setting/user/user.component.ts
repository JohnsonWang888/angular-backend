...
import { UserRestService } from 'delon-restful';
import {
  FormBuilder,
  FormGroup,
  Validators
...
export class UserComponent implements OnInit {
  ...
  constructor(private notification: NzNotificationService, ModalService: NzModalService, private UserService: UserRestService, private fb: FormBuilder) {
  }
  ...
  addEmpowerment(): void {
    ...
    this.UserService.getRoleData().subscribe(res => {
      ...
      this.checkOptionsOne = res.result.map(d => {
        return {
          "label": d.name,
          "value": d.id,
          "checked": false
...
//选择角色名
  ...
  edit(id, name): void {
    ...
    this.UserService.selectRole(id).subscribe(res => {
      // console.log(res);
      this.params = {
        name: res.result.name,
        userName: res.result.username,
      };
      this.checkOptionsOne.map(d => {
        res.result.roles.map(f => {
          if (d.label === f) {
            d.checked = true;
          }
        })
...
//搜索
  ...
  userEmpowerment(data): void {
    ...
    this.UserService.getGroup().subscribe(res => {
      ...
      this.UserService.empowermentAll().subscribe(res => {
        ...
        this.UserService.selectListById(data).subscribe(res => {
          if (res.result) {
  ...
  handleOk(value: any): void {
    ...
    this.rolesPower = this.checkOptionsOne.filter(d => {
      if (d.checked) {
        return d.label
      }
    }).map(c => {
    ...
    let params = {
      ...
      roles: this.rolesPower
    };

    if (this.modify) {
      this.UserService.modify(this.modify, params).subscribe(res => {
        if (res.status === 200) {
          this.addEmpowermentVisible = false;
      ...
      this.UserService.addRole(params).subscribe(res => {
        if (res) {
          this.addEmpowermentVisible = false;
          this.createNotification('success', '添加用户', '添加成功!');
          this.UserService.getUserData().subscribe(res => {
            // console.log(res.result.data);
            this.data = res.result;
          });
        ...
        if (error.error.message == "{username_exists}")
          this.createNotification('error', '添加用户', '用户名已存在!');
